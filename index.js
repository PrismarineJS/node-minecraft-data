...
types.forEach((type) => {
    ...
    preNettyVersionsByProtocolVersion[type] = indexer.buildIndexFromArrayNonUnique(
        protocolVersions[type].filter((e) => {
            return !e.usesNetty;
        }),
        'version'
    );
    postNettyVersionsByProtocolVersion[type] = indexer.buildIndexFromArrayNonUnique(
        protocolVersions[type].filter((e) => {
            return e.usesNetty;
        }),
        'version'
...
const cache = {}; // prevent re-indexing when requiring multiple time the same version
